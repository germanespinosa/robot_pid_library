...
namespace robot {
    bool Tracking_simulator::send_update(const cell_world::Step &step){
        ...
        //simulates noise
        transformed_step.location.x += Chance::dice_float(-noise, noise);
        transformed_step.location.y -= Chance::dice_float(-noise, noise);
        // simulates bad readings
        if (Chance::coin_toss(bad_reads)) { //1% of the updates are garbage
            transformed_step.location = Location(Chance::dice_float(0, 1),Chance::dice_float(0, 1));
        }
